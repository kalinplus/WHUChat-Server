...
{
    static std::size_t idx_next_service = 0;

    IoService& service = io_services[ idx_next_service ];
    // 递增，并处理上溢
    idx_next_service++;
    if ( idx_next_service == io_services.size() )
        idx_next_service = 0;
...
void AsioIoContextPool::Tidy()
{
    ...
    for ( WorkPtr& work : works )
    {
        IoService& service = work->get_executor().context();
        if ( !service.stopped() )
            service.stop(); // 使 io_context 的 run 函数结束阻塞

        work.reset(); // 解绑定 io_context
    }

    // 在 thread 中运行的 io_context 结束 run 后回调函数就已经停止阻塞结束运行可以join
...
AsioIoContextPool::AsioIoContextPool( std::size_t size )
    ...
    /* 此处不初始化 working_threads 的大小，而在下文 emplace_back 动态增加 */
{
    // 将 io_context 绑定到对应的 executor_work_guard
    // executor_work_guard 可以保证 io_context 没有事件注册时不会跳出 run 函数
    // （因为下面在线程刚开始运行的时候肯定没有事件注册）
    ...
    // 遍历每个 io_service，将其分配到一个线程中开始 run
    for ( std::size_t i = 0; i < io_services.size(); i++ )
    {
        working_threads.emplace_back(
            [ this, i ] ()
            {
                ...
                {
                    std::cerr << "AsioIoContextPool线程池（ID: " << i
                        << "）中出现错误: " << e.what() << std::endl;
                }
            } );
    }
}